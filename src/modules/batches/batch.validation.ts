...
const baseBatchSchema = z.object({
  ...
  status: batchStatusEnumSchema,
  metadata: z.record(z.string(), z.any()),
  accessTillDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Must be in YYYY-MM-DD format').optional().nullable(),
  accessTillYear: z.number().int().min(1).optional().nullable(),
});

export const createBatchSchema = baseBatchSchema.extend({
...
export const updateBatchSchema = baseBatchSchema.partial();
