...
describe('AdminContentLibraryService - HLS Signed URL Generation', () => {
  ...
  it('should return signedUrl and expiresAt for getItem when videoLink matches Bunny pull zone', async () => {
    ...
    expect(result.videoLink).toBe(mockItem.videoLink);
    expect(result.signedUrl).toBeDefined();
    expect(result.signedUrl).toContain(`https://${TARGET_BUNNY_PULL_ZONE_HOST}/bcdn_token=HS256-`);
    expect(result.signedUrl).toContain('token_path=%2F143aeeff-e353-4b64-8a8c-204d777bdbcc%2F');
    expect(result.signedUrl).toContain('/143aeeff-e353-4b64-8a8c-204d777bdbcc/playlist.m3u8');
    expect(typeof result.expiresAt).toBe('number');
  });

  it('should return original videoLink as signedUrl if non-Bunny video link', async () => {
    const nonBunnyItem = {
      ...mockItem,
    ...
    expect(result.signedUrl).toBe('https://vimeo.com/123456');
    expect(result.expiresAt).toBeNull();
  });

  it('should return signedUrl as null if videoLink is null', async () => {
    const noVideoItem = {
      ...mockItem,
      videoLink: null,
    };
  ...
  it('should return signedUrl for searchItems list', async () => {
    ...
    expect(result.items[0].signedUrl).toContain(`https://${TARGET_BUNNY_PULL_ZONE_HOST}/bcdn_token=HS256-`);
