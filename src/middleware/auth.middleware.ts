...
export const authMiddleware = (): MiddlewareHandler => {
  return async (c, next) => {
    try {
      ...
      // Normalize IP and lazy-backfill session location data asynchronously from headers (0ms latency)
      const sessionObj = sessionData.session as any;
      if (sessionObj && sessionObj.id) {
        const rawIp = sessionObj.ipAddress || c.req.header('cf-connecting-ip') || c.req.header('x-forwarded-for') || c.req.header('x-real-ip') || '';
        ...
        if (isMappedIp || isLocationMissing || isLocationChanged) {
          ...
          const newCity = incomingCity || sessionObj.city || 'Unknown';
